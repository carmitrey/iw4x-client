...
namespace Components::GamepadControls
{
	...
	//
	// Capabilities are facts a driver has established about a specific
	// physical device, not assumptions about a family. A feature bit is set
	// only when the driver can actually decode or drive it over the current
	// transport; for example a DualSense on Bluetooth advertises haptics
	// differently than on USB, and the driver records what it can genuinely
	// do, not what the model supports in principle.
	...
	//
	// A thin, allocation-free wrapper over the underlying bit set. It exists
	// so that capability queries read as intent (caps.has(capability::rumble))
	...
	public:
		...
		constexpr capabilities& add(capability c) noexcept
		{
			bits_ |= static_cast<rep>(c);
			return *this;
		}

		constexpr capabilities& remove(capability c) noexcept
		{
			bits_ &= ~static_cast<rep>(c);
			return *this;
		}
		...
		constexpr rep value() const noexcept { return bits_; }

		friend constexpr bool operator==(capabilities, capabilities) noexcept = default;

		friend constexpr capabilities operator|(capabilities l, capabilities r) noexcept
		{
			capabilities c;
	...
	constexpr capabilities operator|(capability l, capability r) noexcept
	{
		return capabilities(l) | capabilities(r);
	}

	std::ostream& operator<<(std::ostream&, capabilities);
}
