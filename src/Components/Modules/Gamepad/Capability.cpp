...
namespace Components::GamepadControls
{
	...
	{
		// Render as a brace-enclosed, comma-separated list of the set features
		// so a diagnostic reads as a fact rather than a hexadecimal mask. The
		// order is fixed and independent of insertion order.
		struct entry
		{
			capability bit;
			const char* name;
