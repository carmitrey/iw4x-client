...
{
	bool same_binding(const transport_binding& a, const transport_binding& b) noexcept
	{
		...
		// Both are the unbound monostate; an unbound record identifies no
		// device, so two of them are not "the same device".
		return false;
	}
}
