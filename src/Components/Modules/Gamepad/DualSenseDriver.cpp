...
namespace Components::GamepadControls
{
	namespace
	{
		// Framing constants (drivers/hid/hid-playstation.c, struct
		// dualsense_input_report). The common report begins after the
		// report id over USB and after the report id plus one byte over
		// Bluetooth; the Bluetooth report carries a trailing
		...
		bool resolve_frame(std::span<const std::byte> r, connection link, size_t& common) noexcept
		{
			...
			if (link == connection::bluetooth)
			{
				...
				if (!verify_ps_crc32(ps_input_crc_seed, r.first(ds_size_bt - 4), crc))
					return false;
	...
	bool decode_dualsense(std::span<const std::byte> r, connection link, raw_sample& raw, canonical_sample& canonical, bool edge) noexcept
	{
		size_t b(0);
		...
		// Buttons. buttons[] begins at offset 7: [0] hat + face, [1]
		// shoulders / triggers / create / options / stick clicks, [2] PS
		// / touchpad / mute and, on an Edge, the four rear buttons in
		// bits 4..7.
		...
		// Motion. Raw device counts, converted to physical units by the
		// calibration stage; the device sensor clock is a 32-bit
		// timestamp here.
		...
		// Touchpad: two contacts carried directly in the report (no
		// sub-frames).
		...
		// Battery: status[0] low nibble is the capacity, high nibble the
		// charging state. The percent formulas follow the kernel driver.
		...
		case 0x2:
			...
			break;
		default:
			// Temperature faults and unknown codes carry no usable
			// capacity.
	...
	dualsense_driver::dualsense_driver(hid_device& hid, device_id device)
		: hid_(hid), device_(device), link_(hid.link())
	{
	}

	bool dualsense_driver::read_and_decode(raw_sample& raw, canonical_sample& canonical, bool edge) noexcept
	{
		...
		for (size_t i(0); i != max_reports_per_poll; ++i)
		{
			...
			{
				// Not a malformed report: the device has not left
				// minimal mode, which enable_extended_reports() asks it
				// to do at bind. There is nothing to decode in it, it
				// carries no motion or touchpad data, so drop it, and
				// say so once rather than once per report.
				if (!minimal_reported_)
				{
					minimal_reported_ = true;

					Logger::Print(Game::CON_CHANNEL_SYSTEM,
						"gamepad device {}: DualSense is still sending minimal Bluetooth "
				...
				Logger::Warning(Game::CON_CHANNEL_SYSTEM,
					"gamepad device {}: DualSense report failed framing or CRC validation and "
					"was dropped\n",
					device_.value());
		}
	...
	bool dualsense_driver::poll(raw_sample& raw, canonical_sample& canonical) noexcept
	{
		return read_and_decode(raw, canonical, false);
	}

	void dualsense_driver::submit(const output_request& request) noexcept
	{
		// Rumble, light bar, and player LEDs are encoded against the
		// verified layout; adaptive-trigger effects are not, and the
		// encoder rejects them rather than guess a byte sequence that
		// could put the controller in a bad state.
		std::array<std::byte, ds_output_bt_size> buf;

		const std::optional<size_t> n(encode_dualsense_output(request, link_, bt_output_sequence_, buf));

		if (!n)
			return;

		if (!hid_.write(std::span<const std::byte>(buf.data(), *n)))
			Logger::Warning(Game::CON_CHANNEL_SYSTEM, "gamepad device {}: DualSense output report write failed\n", device_.value());
	}
}
