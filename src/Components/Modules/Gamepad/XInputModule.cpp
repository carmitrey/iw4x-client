...
namespace Components::GamepadControls
{
	...
	void xinput_module::load()
	{
		// Try the variants in descending order of capability. The reason each
		// one is in the list, and where it comes from, is what a future
		// maintainer needs to know before reordering or removing an entry:
		//
		...
		for (const char* name : variants)
		{
			...
			// Without these three the library is unusable; treat it as if it
			// had not loaded and try the next variant rather than
			// half-initializing.
			...
			// XInputGetStateEx is exported by ordinal 100 and has no public
			// name. It is identical to XInputGetState except that it does not
			// clear the guide button bit. Resolving it is best-effort: its
			// absence only means the guide button cannot be decoded, not that
			...
			Logger::Print(Game::CON_CHANNEL_SYSTEM, "XInput loaded via {} ({})\n", name,
				get_state_ex != nullptr ? "guide button available" : "no guide button");
			return;
		}

		Logger::Warning(Game::CON_CHANNEL_SYSTEM,
			"no XInput runtime could be loaded; XInput controllers will not be available\n");
	}

	DWORD xinput_module::get_state(DWORD user_index, XINPUT_STATE& out) const noexcept
	{
	...
	DWORD xinput_module::get_capabilities(DWORD user_index, DWORD flags, XINPUT_CAPABILITIES& out) const noexcept
	{
	...
	DWORD xinput_module::set_state(DWORD user_index, XINPUT_VIBRATION& vibration) const noexcept
	{
