...
{
	std::optional<aim_graph> aim_graph::make(std::span<const knot> ks, bool require_monotonic, std::string& why)
	{
		if (ks.size() < 2)
		{
	...
	float aim_graph::evaluate(float input) const noexcept
	{
		// The class invariant guarantees at least two knots with
		// strictly increasing inputs; evaluate relies on it rather than
		// re-validating.
		assert(count_ >= 2);

		const knot& first(knots_[0]);
		const knot& last(knots_[count_ - 1]);
		...
		// Locate the segment [i-1, i] containing input and interpolate
		// linearly. The span is strictly positive by the
		// increasing-inputs invariant, so the slope is well defined.
