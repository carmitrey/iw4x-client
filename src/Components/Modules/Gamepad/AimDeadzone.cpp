...
{
	bool validate(const deadzone_params& p, std::string& why) noexcept
	{
	...
	stick_vector apply(const deadzone_params& p, stick_vector v) noexcept
	{
		const float inner(p.inner.value);
		const float outer(p.outer.value);
		const float anti(p.anti.value);

		// The parameters have already been validated; assert the
		// invariant the rest of the function depends on rather than
		// re-checking external data here.
		...
		// Rescale the vector to the new magnitude while preserving
		// direction. m > 0 here because m > inner >= 0.
