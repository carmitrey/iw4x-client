#include "CalibrationNormalize.hpp"
...
{
	void apply(const profile& p, const raw_sample& raw, canonical_sample& canonical) noexcept
	{
		// Sticks: centre and range correction on the driver's normalized
		// vector.
		...
		// Triggers: rescale the normalized value to the measured travel,
		// replacing the driver's straight normalization.
		for (size_t i(0); i < trigger_count; ++i)
		{
			...
			const float v(denom > 0.0f ? (t.normalized - tc.min) / denom : t.normalized);
			t.normalized = std::clamp(v, 0.0f, 1.0f);
		}

		// Motion: debias and scale the raw counts into physical units.
		// Only produced when the device actually reported motion.
		if (raw.motion)
		{
