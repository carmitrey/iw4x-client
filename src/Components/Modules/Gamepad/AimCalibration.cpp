...
namespace Components::GamepadControls
{
	float fov_scale(degrees fov, degrees reference_fov) noexcept
	{
		...
		// Guard a degenerate or non-finite reference so the scale is
		// always usable.
	...
	std::optional<aim_calibration> aim_calibration::make(const aim_settings& s, std::string& why)
	{
		std::string w;
		...
		if (s.graph_knots)
		{
			std::optional<aim_graph> built(
				aim_graph::make(std::span<const knot>(s.graph_knots->data(), s.graph_knots->size()), s.graph_monotonic, w));
			if (!built)
			{
				why = "aim graph: " + w;
				return std::nullopt;
	...
	aim_processor::config aim_calibration::processor_config() const noexcept
	{
