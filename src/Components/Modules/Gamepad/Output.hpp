...
namespace Components::GamepadControls
{
	...
	//
	// Both intensities are normalized to [0, 1]; the driver scales them to
	// the device's own range. On the DualSense the same request drives the
	// voice-coil actuators, so rumble is the common output every supported
	// family honors.
	...
	//
	// The request is device-neutral: it says what effect to produce, not
	// how to encode it. The driver translates it into the device's raw
	// trigger-effect bytes, which is exactly the encoding that must never
	// be guessed, so it lives behind the driver rather than in this
	// vocabulary.
	//
	// Not yet encoded by any driver in this port (see IW-2.10); the type
	// exists now so output_request already has its final shape.
	...
	//
	// A variant rather than a bag of optional fields, so a driver matches
	// on the one thing it is being asked to do and rejects what it cannot
	// honor. There is deliberately no "raw report" alternative: nothing
	// ever hands a driver bytes to emit blindly.
	using output_request = std::variant<rumble_request, light_bar_request, player_led_request, adaptive_trigger_request>;
}
