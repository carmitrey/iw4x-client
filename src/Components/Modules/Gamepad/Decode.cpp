...
{
	uint8_t rd_u8(std::span<const std::byte> d, size_t off) noexcept
	{
		assert(off < d.size());
		return static_cast<uint8_t>(d[off]);
	}

	uint16_t rd_le16(std::span<const std::byte> d, size_t off) noexcept
	{
		assert(off + 1 < d.size());
	...
	int16_t rd_le16s(std::span<const std::byte> d, size_t off) noexcept
	{
		return static_cast<int16_t>(rd_le16(d, off));
	}

	uint32_t rd_le32(std::span<const std::byte> d, size_t off) noexcept
	{
		assert(off + 3 < d.size());
	...
	uint32_t crc32_le(uint32_t crc, std::span<const std::byte> data) noexcept
	{
		// Reflected bit-at-a-time CRC-32 (IEEE 802.3). A table would be
		// faster, but a Bluetooth report is at most 78 bytes and this runs
		// once per report, so the simpler form is kept for auditability
	...
	void apply_hat(button_set& s, uint8_t hat) noexcept
	{
		// Direction vectors for hat values 0..7; 8 (and anything else) is
		// released. Index order matches the device encoding: 0 is up,
		// advancing clockwise.
		struct dir
		{
			int x;
	...
	{
		// Map the unsigned byte centred at 128 onto [-1, 1] using 127 as
		// full scale so that 255 reaches +1 and 1 reaches -1 symmetrically.
		// Invert Y so up is positive, matching the canonical convention.
		float fx(std::clamp((static_cast<float>(x) - 128.0f) / 127.0f, -1.0f, 1.0f));
		float fy(std::clamp((128.0f - static_cast<float>(y)) / 127.0f, -1.0f, 1.0f));
