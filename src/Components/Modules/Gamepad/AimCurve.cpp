...
{
	const char* to_string(curve_kind k) noexcept
	{
		switch (k)
		{
	...
	bool validate(const response_curve& c, std::string& why) noexcept
	{
	...
	float evaluate(const response_curve& c, float t) noexcept
	{
		...
		case curve_kind::power:
			// 0 and 1 are fixed points of a power curve for any positive
			// exponent, so the range is preserved exactly at the
			// endpoints.
