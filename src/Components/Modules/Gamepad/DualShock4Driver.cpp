...
namespace Components::GamepadControls
{
	namespace
	{
		// Framing constants (drivers/hid/hid-playstation.c). The common
		// report block is 32 bytes; it begins after the report id over USB
		// and after the report id plus a two-byte header over Bluetooth.
		// The Bluetooth report carries a trailing little-endian CRC-32
		...
		// Resolve the framing for a report, returning the offset of the
		// common block and the maximum number of touch sub-frames.
		// Validates the report id, length, and (Bluetooth) CRC against
		// the raw bytes before anything is decoded; returns false to
		// reject a report that does not match its framing rather than
		// risk associating it with the wrong layout.
		bool resolve_frame(std::span<const std::byte> r, connection link, size_t& common, size_t& max_touch) noexcept
		{
			...
			if (link == connection::bluetooth)
			{
				...
				if (!verify_ps_crc32(ps_input_crc_seed, r.first(ds4_size_bt - 4), crc))
					return false;
	...
	bool decode_dualshock4(std::span<const std::byte> r, connection link, raw_sample& raw, canonical_sample& canonical) noexcept
	{
		size_t b(0);
		size_t max_touch(0);
		...
		// Buttons. buttons[0] holds the hat and the four face buttons;
		// buttons[1] the shoulders, triggers, share/options, and stick
		// clicks; buttons[2] the PS and touchpad-click buttons (the
		// remaining bits are a report counter).
		...
		canonical.triggers[static_cast<size_t>(trigger_side::left)] = {z, static_cast<float>(z) / 255.0f};
		canonical.triggers[static_cast<size_t>(trigger_side::right)] = {rz, static_cast<float>(rz) / 255.0f};

		// Motion. Retained as raw device counts in the raw sample; the
		// calibration stage converts these into the canonical sample's
		// physical units using the device's calibration feature report.
		...
		// Battery. status[0] holds the capacity in its low nibble and the
		// cable state in bit 4; a capacity of 11 with the cable attached
		// means full.
		...
		{
			bat.state = cable ? battery_state::status::charging : battery_state::status::discharging;
			bat.percent = static_cast<uint8_t>(std::min<int>(cap * 10, 100));
		}
		canonical.battery = bat;
		raw.battery = bat;

		// Touchpad. The report carries several chronological sub-frames;
		// a per-engine-frame snapshot needs only the current contact
		// state, so the first sub-frame is decoded when the report
	...
	dualshock4_driver::dualshock4_driver(hid_device& hid, device_id device)
		: hid_(hid), device_(device), link_(hid.link())
	{
	}

	bool dualshock4_driver::poll(raw_sample& raw, canonical_sample& canonical) noexcept
	{
		// The device reports faster than the engine frames, so drain what
		// has arrived and keep the newest valid report: an engine frame
		// wants the controller's current position, not a queue of stale
		// ones. The bound keeps a device that reports pathologically fast
		// from stalling the frame; leftover reports are simply drained on
		// the next one.
		...
		for (size_t i(0); i != max_reports_per_poll; ++i)
		{
			...
			{
				// Not a malformed report: the device has not left minimal
				// mode, which enable_extended_reports() asks it to do at
				// bind. There is nothing to decode in it, it carries no
				// motion or touchpad data, so drop it, and say so once
				...
				if (!minimal_reported_)
				{
					...
					Logger::Print(Game::CON_CHANNEL_SYSTEM,
						"gamepad device {}: DualShock 4 is still sending minimal Bluetooth "
						"reports and produces no input; the controller did not switch to its "
						"extended report\n",
						device_.value());
				}
			}
				...
				Logger::Warning(Game::CON_CHANNEL_SYSTEM,
					"gamepad device {}: DualShock 4 report failed framing or CRC validation "
					"and was dropped\n",
					device_.value());
		}
	...
	void dualshock4_driver::submit(const output_request& request) noexcept
	{
		// Encode into a stack buffer sized for the larger (Bluetooth)
		// report; the encoder returns the actual length and rejects a
		// request the device and link cannot honor rather than emitting
		// anything uncertain.
		std::array<std::byte, ds4_output_bt_size> buf;

		const std::optional<size_t> n(encode_dualshock4_output(request, link_, buf));

		if (!n)
			return;

		if (!hid_.write(std::span<const std::byte>(buf.data(), *n)))
			Logger::Warning(Game::CON_CHANNEL_SYSTEM, "gamepad device {}: DualShock 4 output report write failed\n", device_.value());
	}
}
